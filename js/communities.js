...
export async function loadUserCommunities(container, onSelectCommunity) {
    ...
    snapshot.forEach(docSnap => {
        ...
        div.innerHTML = `<div class="w-10 h-10 rounded-full bg-gradient-to-tr from-cyan-500 to-purple-700 flex items-center justify-center"><i class="fas fa-layer-group text-white"></i></div><span class="channel-name text-sm">${escapeHTML(comm.name)}</span>`;
    ...
    addBtn.innerHTML = `<div class="w-10 h-10 rounded-full bg-emerald-500/30 flex items-center justify-center"><i class="fas fa-plus text-emerald-300"></i></div><span class="channel-name text-sm">إنشاء مجتمع</span>`;
...
async function createCommunity() {
    ...
    showLoading(true);
    try {
        await addDoc(collection(db, "communities"), { name: name.trim(), description: desc || "", ownerId: currentUser.uid, members: [currentUser.uid], createdAt: serverTimestamp() });
        showToast(`تم إنشاء المجتمع "${name}" بنجاح!`, 'success');
    } catch (error) { showToast('خطأ في إنشاء المجتمع', 'error'); }
